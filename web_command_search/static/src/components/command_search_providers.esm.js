...
class CommandSearchCommandComponent extends Component {}
CommandSearchCommandComponent.template = "CommandSearchPalette";
CommandSearchCommandComponent.props = {
...
const fn = (search) => {
    ...
    return async function provide(env, options) {
        ...
        if (options.searchValue !== "" && data !== false) {
            ...
            items.forEach((item) => {
                result.push({
                    Component: CommandSearchCommandComponent,
                    action() {
