...
class HiddenTemplateField(models.Model):
    ...
    groups = fields.Many2many(
        ...
        help="Is you don't select any group, the field" "is hidden to all groups",
    )
    active = fields.Boolean(related="template_id.active")
    model = fields.Many2one(comodel_name="ir.model", related="template_id.name")
...
class HiddenTemplate(models.Model):
    ...
    name = fields.Many2one(comodel_name="ir.model", string="Model", required=True)
